...
async function main() {
  const found = await db.select({ id: players.id, name: players.name, pos: players.position, country: countries.name })
    ...
    .where(ilike(players.name, "%demb%"));
  console.log("Candidatos:", found);
  const dembele = found.find((p) => p.country === "France") ?? found[0];
  if (!dembele) { console.log("No encontrado"); return; }
  console.log("\n=> Usando:", dembele);
  ...
  const erp = await db.select().from(entryRoundPlayers)
    .where(and(inArray(entryRoundPlayers.entryRoundId, ers.map((e)=>e.id)), eq(entryRoundPlayers.playerId, dembele.id)));

  console.log(`\n=== En Zonallaunior, ${dembele.name} en F3 ===`);
  ...
  for (const r of erp) {
    ...
    const cap = info.cap === dembele.id ? " (CAPITÁN)" : "";
