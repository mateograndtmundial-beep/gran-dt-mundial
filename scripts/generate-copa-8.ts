...
/*
 * Publicación #8 del lanzamiento de la Liga Premium (docs/social/LANZAMIENTO-COPA.md §"8 · D-1").
 * "Última chance: cierra mañana." Carrusel 2 placas 1080×1350. Pico del empuje de 48 h.
 * Figura por slide: hero "1 DÍA" (anillo gigante + recap de la oferta, tono dark/urgente) → cupo en
 * vivo (barra) + CTA "último día para anotarte". Más intenso que la #7 (rojo dominante).
 ...
 *
 *   npx tsx scripts/generate-copa-8.ts --enrolled 91   # 91 anotados → quedan 9
 *   npx tsx scripts/generate-copa-8.ts                 # placeholder (avisa por consola)
 */
...
const ENROLLED = Math.max(0, Math.min(CAPACITY, Number(ENROLLED_RAW ?? 91)));
...
// stat compacto: numeral + label (para el recap dentro del hero)
...
// 1 · Hero: cierra mañana (anillo "1 DÍA" + recap de la oferta)
function slide1() {
  return `<div class="content" style="margin-top:34px;">
    <div style="display:flex;align-items:center;gap:10px;">${ICON.alarm(26, "#D02B2B")}<span class="eyebrow" style="color:var(--danger);">Última chance · cierra mañana</span></div>
    <div class="title" style="font-size:114px;margin-top:12px;">CIERRA<br><span class="r">MAÑANA</span>.</div>
    <p class="body" style="font-size:29px;line-height:1.36;margin-top:18px;max-width:940px;">Mañana cierra la inscripción a la Liga Premium y <b>no se reabre</b>. Si lo venías pensando, es <b>ahora</b>.</p>

    <div style="margin-top:34px;background:var(--dark);border:2px solid var(--gold);border-radius:18px;box-shadow:9px 9px 0 rgba(17,24,39,.55);padding:40px 40px 34px;">
      <div style="display:flex;align-items:center;justify-content:center;gap:34px;">
        <div style="position:relative;flex:0 0 auto;width:188px;height:188px;border-radius:50%;background:linear-gradient(135deg,#DDBB60,#C8A24B);border:5px solid var(--gold);display:flex;align-items:center;justify-content:center;box-shadow:0 10px 0 rgba(0,0,0,.28);">
          <span class="num" style="font-size:120px;color:var(--ink);">1</span>
          <div style="position:absolute;top:-14px;right:-14px;width:60px;height:60px;border-radius:50%;background:var(--danger);border:3px solid var(--dark);display:flex;align-items:center;justify-content:center;">${ICON.alarm(32, "#fff")}</div>
        </div>
        <div style="text-align:left;">
          <div class="num" style="font-size:82px;color:#fff;">DÍA</div>
          <div class="eyebrow" style="font-size:19px;color:var(--gold);margin-top:6px;">para entrar</div>
          <div style="font-weight:700;font-size:21px;color:#9AA6BF;margin-top:10px;max-width:320px;line-height:1.3;">Después se cierra al rodar la pelota en 16vos.</div>
        </div>
      </div>
      <div style="display:flex;gap:10px;margin-top:38px;padding-top:30px;border-top:1px solid rgba(255,255,255,.1);">
  ...
  <div style="position:absolute;left:60px;right:60px;bottom:128px;z-index:2;background:var(--danger-bg);border:2px solid var(--danger);border-radius:16px;padding:20px 38px;display:flex;align-items:center;justify-content:space-between;box-shadow:7px 7px 0 var(--danger-border);">
    <div><div class="eyebrow" style="font-size:15px;color:var(--danger);">Mañana ya es tarde</div>
    <div style="color:var(--ink);font-weight:800;font-size:30px;line-height:1.05;margin-top:4px;">Decidilo hoy</div></div>
...
// 2 · Cupo en vivo (barra) + CTA: último día para anotarte
function slide2() {
  return `<div class="content" style="margin-top:30px;">
    <div class="eyebrow">El último llamado útil</div>
    <div class="title" style="font-size:62px;margin-top:8px;">ENTRÁ HOY,<br><span class="r">MAÑANA NO</span>.</div>
    <p class="body" style="font-size:26px;line-height:1.32;margin-top:14px;max-width:940px;">Quedan <b>${LEFT} de ${CAPACITY}</b> lugares. Los que entran juegan por <b>$400.000</b> garantizados al top 10.</p>
    ...
    <div class="card" style="margin-top:24px;padding:26px 30px;">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:10px;">
        ...
        <span class="num" style="font-size:30px;color:var(--danger);">${PCT}%</span>
      </div>
      ...
      <div style="font-weight:800;font-size:19px;color:var(--danger);margin-top:10px;">Casi lleno · quedan ${LEFT} y la inscripción no se reabre.</div>
    </div>

    <div style="display:flex;flex-direction:column;gap:14px;margin-top:22px;">
      ${conf(ICON.lock(38), "Mañana cierra y no se reabre", "Al rodar la pelota en 16vos. No hay repechaje ni lista de espera.")}
    ...
    <div style="margin-top:30px;background:var(--dark);border:2px solid var(--gold);border-radius:16px;box-shadow:9px 9px 0 rgba(17,24,39,.6);padding:38px 42px;text-align:center;">
      <div style="color:#fff;font-weight:800;font-size:28px;">Último día para anotarte</div>
...
async function main() {
  ...
  const outDir = path.join(ROOT, arg("out") ?? "out/copa-8");
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    const file = path.join(outDir, `copa-8_${String(i + 1).padStart(2, "0")}.png`);
