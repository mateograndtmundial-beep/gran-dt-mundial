 ...
 *   "Mañana 16:00 (ARG) arrancan los 16VOS del Mundial y se cierra la ventana
 *    de cambios". Faltan menos de 24 hs.
 *
 ...
 *
 *   npx tsx scripts/generate-story-16vos.ts   # → out/reminders/
 */
...
const BLUE: Tones = { accent: "#1B4FD8", accentBg: "var(--blue-light)", accentBorder: "var(--blue-border)" };

const html = `
  ${eyebrow(ic(P.alarm, "#1B4FD8", 30), "Última oportunidad", "#1B4FD8")}
  <div class="title" style="font-size:100px;margin-top:16px;">HACÉ TUS<br><span class="b">CAMBIOS</span><br>ANTES DE MAÑANA</div>
  <p class="body" style="font-size:32px;line-height:1.36;margin-top:24px;">Mañana arrancan los 16vos y <b>se cierra la edición</b>. Ajustá tu 11 y elegí capitán antes de que empiece la fecha.</p>
  ${heroNumeralCard(
    P.lock,
    "Cierra mañana a las",
  ...
  <div style="display:flex;flex-direction:column;gap:16px;margin-top:34px;">
    ${iconCard(ic(P.swap, "#1B4FD8"), "Es tu ventana de cambios", "Ajustá el 11 y los suplentes para los 16vos.", "var(--blue-light)", "var(--blue-border)")}
    ${iconCard(ic(P.lock, "#1B4FD8"), "Falta menos de 24 hs", "Después del pitazo inicial no se toca nada más.", "var(--blue-light)", "var(--blue-border)")}
  </div>
...
const foot: Foot = { url: "LOS11DESAMPA.COM", tag: "16vos · Mundial" };
...
async function main() {
  ...
  const file = path.join(outDir, "story_16vos_cierre.png");
