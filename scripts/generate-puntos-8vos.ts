...
/*
 * Carrusel 3 slides 1080×1350 — "YA ESTÁN LOS PUNTOS · 8VOS DE FINAL"
 ...
 *
 * Correr DESPUÉS de publishRound(8vos).
 *   npx tsx scripts/generate-puntos-8vos.ts   → out/puntos-8vos/
 */
...
async function getData() {
  const fecha3 = (await db.select().from(rounds).where(eq(rounds.order, 5)).limit(1))[0];
  if (!fecha3) throw new Error("8vos de Final no encontrada");
  ...
  *{margin:0;padding:0;box-sizing:border-box;font-family:'Poppins',sans-serif;}
  ...
  <div class="wrap">
    ...
    <div style="position:relative;z-index:2;display:flex;flex-direction:column;justify-content:center;height:1180px;gap:26px;">
      <div class="eyebrow">8VOS DE FINAL · MUNDIAL 2026</div>
...
async function main() {
  ...
  const outDir = path.join(ROOT, "out/puntos-8vos");
  await mkdir(outDir, { recursive: true });

  void equipos; // portada limpia: no muestra stat pills

  const slides = [
    { slide: 1, html: coverDoc(logoB64, titleFontCss, "RESUMEN · 8VOS") },
    { slide: 2, html: doc(logoB64, titleFontCss, 2, slide2(topFecha, topGlobal), "TOP · 8VOS") },
    { slide: 3, html: doc(logoB64, titleFontCss, 3, slide3(xi, xiTotal, flags), "MEJOR XI · 8VOS") },
  ];

  for (const s of slides) {
    const buf = await renderPng(s.html, SIZE);
    const file = path.join(outDir, `puntos_8vos_0${s.slide}.png`);
